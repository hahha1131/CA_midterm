...
* 포드 풀커슨 알고리즘의 결과는 소스코드를 작성하지 못하여 직접 캡처를 할 수 없었다. 
* 이론상으로는 에드몬드 카프 알고리즘에서 사용한 형식을 사용하면 20000번의 탐색을 거쳐서 전체 유량을 출력하게 될 것이다.
...
* 앞서 내가 작성한 소스코드를 기준으로 설명하면 중앙 간선의 용량을 1로 설정하고 다른 간선의 용량을 10000으로 설정했을 때, BFS 방식을 사용하는 에드몬드 카프 알고리즘은 단 2번의 연산으로 전체 유량을 찾아내었다.
* 그러나, 작성하지는 못했지만 DFS 방식을 사용하는 포드 풀커슨 알고리즘은 20000번의 연산을 거쳐서 전체 유량을 찾게 될 것이다.
* 간선이 적으면서, 유량이 많은 경우에는 BFS를 사용하는 에드몬드 카프 알고리즘이
* 간선이 많은데, 유량이 적은 경우에는 DFS를 사용하는 포드 풀커슨 알고리즘이 더 적합하다는 것을 알 수 있다.
...
* 오래간만에 예전에 작성했던 인접리스트, 그래프, DFS, BFS 관련 소스코드들을 찾아보는 교육적인 시간이었던 느낌이다. 정보를 찾고 이해하고 소스코드를 작성하기까지 오래 걸렸지만 다양한 알고리즘 지식을 쌓을 수 있었고 java를 C와 같은 수준까지는 아니더라도 좀 더 편하게 다룰 수 있도록 많이 활용해볼 수 있는 시간이었다.
